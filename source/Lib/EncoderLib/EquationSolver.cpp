...
void EquationSolver::scale_mat(int64_t *_M, int64_t *_D, int64_t *_L, int64_t *_C, uint8_t *_B)
{
    ...
    if (MD_BITS >= LC_BITS) {
        if (MD_BITS > COMP_MAX_BITS) {
            DIFF_BITS = MD_BITS - COMP_MAX_BITS + 1;
        }
    } else {
        if (LC_BITS > COMP_MAX_BITS) {
            DIFF_BITS = LC_BITS - COMP_MAX_BITS + 1;
        }
    }

    if (DIFF_BITS >= 1) {
        *_M = (*_M < 0) ? -(-*_M >> (DIFF_BITS >> 1)) : *_M >> (DIFF_BITS >> 1);
        *_D = (*_D < 0) ? -(-*_D >> (DIFF_BITS >> 1)) : *_D >> (DIFF_BITS >> 1);

        *_L = (*_L < 0) ? -(-*_L >> (DIFF_BITS >> 1)) : *_L >> (DIFF_BITS >> 1);
        *_C = (*_C < 0) ? -(-*_C >> (DIFF_BITS >> 1)) : *_C >> (DIFF_BITS >> 1);
